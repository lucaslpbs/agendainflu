...
const CadastroInfluenciadora = () => {
  ...
  const generateUsername = (nome: string) =>
    nome.toLowerCase().replace(/\s+/g, "").replace(/[^a-z0-9]/g, "") + Math.floor(Math.random() * 100);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const username = generateUsername(form.nome);
      const { error: profileError } = await supabase.from("influencers").insert({
