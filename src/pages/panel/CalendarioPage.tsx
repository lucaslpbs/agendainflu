...
const CalendarioPage = () => {
  ...
  const fetchData = async () => {
    ...
    const [avRes, bkRes] = await Promise.all([
      ...
      supabase.from("bookings").select("*, clients(*), services(*)").eq("influencer_id", influencer.id).gte("data_agendada", start).lte("data_agendada", end),
    ]);
    setAvailability(avRes.data || []);
    setBookings((bkRes.data as any) || []);
  };
  ...
  const toggleBlock = async (date: string) => {
    ...
    if (existing) {
      await supabase.from("availability").update({ bloqueado: !existing.bloqueado }).eq("id", existing.id);
    } else {
      await supabase.from("availability").insert({ influencer_id: influencer.id, data: date, slots_disponiveis: 1, bloqueado: true });
    }
    fetchData();
  };

  const saveSlots = async () => {
    if (!influencer || !selectedDate) return;
    const existing = availability.find((a) => a.data === selectedDate);
    if (existing) {
      await supabase.from("availability").update({ slots_disponiveis: editingSlots }).eq("id", existing.id);
    } else {
      await supabase.from("availability").insert({ influencer_id: influencer.id, data: selectedDate, slots_disponiveis: editingSlots });
    }
